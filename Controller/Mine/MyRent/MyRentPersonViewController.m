...
#define menu_max   999
@interface MyRentPersonViewController (){
...
@implementation MyRentPersonViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    currentSelectMenuIndex = self.selectIndex;
...
//    0=全部，1-申请中/待同意，2-已同意/待见面，3-拒绝,4-待评价，5-异常，6-完成，7-取消,8=已评价
    ...
    if (currentSelectMenuIndex == 3) {
        ...
        currentIndex = 3;
    }else if (currentSelectMenuIndex == 6){
        currentIndex = 7;
    }else if (currentSelectMenuIndex == 7){
        currentIndex = 6;
    }
    [self.viewModel fetchMyRentOrderWithToken:[self getToken] status:@(currentIndex) tag:@(1) page:@(++ self.pageIndex)];
    ...
    [myRentPersonView setSecondReturnBlock:^(NSInteger state){
        if (state == 1) {//state==1 取消订单
                ...
                [alertView setReturnBlock:^(NSString *str) {
                    ...
                    RentOrderModel *model = self.itemArr[section];
                    [viewModel cancelMyRentOrderWithToken:[self getToken] Id:model.Id reason:str];
                }];
                [self.view addSubview: alertView];
        }
        if (state == 2) {//state==2 确认已见面
            MyRentViewModel *viewModel = [[MyRentViewModel alloc] init];
            [viewModel setBlockWithReturnBlock:^(id returnValue) {
                [self setupRefresh];
            } WithErrorBlock:^(id errorCode) {
            ...
            RentOrderModel *model = self.itemArr[section];
            [viewModel comfirmMeetWithToken:[self getToken] Id:model.Id type:@(1)];
        }
        if (state == 4 || state == 5) {//state==4，5 评价
            ...
            [remarkView setReturnBlock:^(NSString *str,NSInteger point) {
                ...
                RentOrderModel *model = self.itemArr[section];
                [viewModel remarkMyRentWithToken:[self getToken] Id:model.Id point:@(point) remark:str type:1];
            }];
            [self.view addSubview: remarkView];
        }
    }];
    RentOrderModel *model = self.itemArr[section];
    [myRentPersonView initView:model type:1];
