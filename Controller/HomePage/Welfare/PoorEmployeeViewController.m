...
@interface PoorEmployeeViewController ()<UICollectionViewDelegate,UICollectionViewDataSource>
@property (weak, nonatomic) IBOutlet UICollectionView *itemCollectionView;
@property (nonatomic, retain) NSMutableArray *goodsArr;
@property (nonatomic, retain) NSMutableArray *bannerArr;
@end

@implementation PoorEmployeeViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.goodsArr = [[NSMutableArray alloc]init];
    self.bannerArr = [[NSMutableArray alloc]init];
    [self.itemCollectionView registerNib:[UINib nibWithNibName:@"WelfarePoorCollectionViewCell" bundle:nil] forCellWithReuseIdentifier:@"WelfarePoorCollectionViewCell"];
    [self.itemCollectionView registerNib:[UINib nibWithNibName:@"WelfareScrollCollectionViewCell" bundle:nil] forCellWithReuseIdentifier:@"WelfareScrollCollectionViewCell"];
    [self.itemCollectionView registerClass:[UICollectionReusableView class] forSupplementaryViewOfKind:UICollectionElementKindSectionHeader withReuseIdentifier:@"UICollectionViewHeader"];
    [self.itemCollectionView registerClass:[UICollectionReusableView class] forSupplementaryViewOfKind:UICollectionElementKindSectionHeader withReuseIdentifier:@"UICollectionViewHeaderTwo"];
    ...
    [viewModel getConvertMainPageWithToken:[self getToken] level:@"1"];
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section == 0) {
        WelfarePoorCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"WelfarePoorCollectionViewCell" forIndexPath:indexPath];
            [cell initCellWithData:self.goodsArr[indexPath.row]];
        return cell;
    }
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout*)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section == 0) {
         ...
         return  CGSizeMake(ScreenWidth/3, ScreenWidth/320*130);
    }
...
-(void)skipToEmplloyeeInfo{
    PoorEmpListViewController *vc = [[PoorEmpListViewController alloc]init];
    [self.navigationController pushViewController:vc animated:YES];
}
