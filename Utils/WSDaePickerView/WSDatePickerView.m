...
@property (nonatomic, copy) NSString *daStr;

@end

@implementation WSDatePickerView

-(instancetype)initWithDateStyle:(WSDateStyle)datePickerStyle dateStr:(NSString *)dateStr CompleteBlock:(void(^)(NSDate *))completeBlock {
    self = [super init];
    self.daStr = [NSString stringWithString:dateStr];
    if (self) {
...
//            NSString *dateStr = [NSString stringWithFormat:@"%@ 23:59",str];
//            self.maxLimitDate = [NSDate date:dateStr WithFormat:@"yyyy-MM-dd HH:mm"];
//        }
//        //最小限制
//        if (!self.minLimitDate) {
//            self.minLimitDate = [NSDate date:self.daStr WithFormat:@"yyyy-MM-dd HH:mm"];
//        }
...
-(void)defaultConfig {
    ...
    //最大最小限制
    if (self.maxLimitDate) {
        NSString *str = [self.daStr substringToIndex:10];
        NSString *dateStr = [NSString stringWithFormat:@"%@ 23:59",str];
        self.maxLimitDate = [NSDate date:dateStr WithFormat:@"yyyy-MM-dd HH:mm"];
    }
    //最小限制
...
//        label.textColor =  RGB(247, 133, 51);
        label.textColor = [UIColor colorWithHexString:@"6398f1"];
