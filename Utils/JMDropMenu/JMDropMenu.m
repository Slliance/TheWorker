...
- (UITableView *)tableView {
    if (!_tableView) {
        _tableView = [[UITableView alloc] initWithFrame:CGRectMake(10, 8, self.frame.size.width - 10, self.frame.size.height - 8) style:UITableViewStylePlain];
...
- (void)layoutSubviews {
    ...
    if (self.model.layoutType == JMDropMenuLayoutTypeTitle) {
        ...
        self.titleL.frame = CGRectMake(10, self.screenCenter - 10, self.contentView.frame.size.width - 20, 20);
    }
...
- (void)setModel:(JMDropMenuModel *)model {
    ...
    if (model.layoutType == JMDropMenuLayoutTypeTitle) {
        self.titleL.frame = CGRectMake(0, self.screenCenter - 10, self.frame.size.width, 20);
    } else {
